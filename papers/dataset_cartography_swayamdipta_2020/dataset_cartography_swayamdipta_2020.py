...
table_2_cols = pd.MultiIndex.from_product(
    [datasets, statistics], names=["first", "second"]
)
...
for dataset in datasets:
    table_2_df[dataset, "Std Err"] = table_2_df[dataset]["Std Dev"] / np.sqrt(
        table_2_df[dataset]["Num Samples"]
    )
...
for dataset in datasets:
    ...
    xerr = table_2_dataset_df["Std Err"]
    ...
    plt.xlabel("Accuracy")
    plt.title(
        "Fig 2 from Swayamdipta et al.'s Dataset Cartography (2020)\nDataset: "
        + dataset
    )
